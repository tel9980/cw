...
def order_strategy(draw, customer_id, customer_name):
    ...
    return ProcessingOrder(
        ...
        processes=draw(
            st.lists(
                st.sampled_from(list(ProcessType)), min_size=1, max_size=4, unique=True
            )
        ),
...
class TestProperty3_OutsourcedProcessingAssociation:
    ...
    **验证: 需求 1.3, 3.3**
    ...
    def test_outsourced_processing_order_association(
        self, db_manager, supplier_data, process_count
    ):
        """
        属性: 委外加工记录与订单的关联性
        ...
        for processing in order_processing:
            assert processing.order_id == order.id, "委外加工记录应该关联到正确的订单"
    ...
    @settings(max_examples=50, deadline=None)
    def test_outsourced_processing_supplier_association(
        self, db_manager, supplier_data, quantity, unit_price
    ):
        """
        属性: 委外加工记录与供应商的关联性
        ...
        supplier_processing = processing_manager.list_processing_by_supplier(
            supplier_data.id
        )

        assert len(supplier_processing) >= 1, "应该能够通过供应商ID查询到委外加工记录"
    ...
    @settings(max_examples=50, deadline=None)
    def test_outsourced_processing_cost_calculation(
        self, db_manager, supplier_data, quantity, unit_price
    ):
        """
        属性: 委外加工费用计算准确性
    ...
    @settings(max_examples=50, deadline=None)
    def test_order_total_outsourcing_cost(
        self, db_manager, supplier_data, process_count
    ):
        """
        属性: 订单委外加工总成本计算准确性
    ...
    @settings(max_examples=50, deadline=None)
    def test_outsourced_processing_payment_tracking(
        self, db_manager, supplier_data, quantity, unit_price, payment_amount
    ):
        """
        属性: 委外加工付款跟踪准确性
        ...
        # 验证: 已付金额 + 未付金额 = 总成本
        assert (
            updated.paid_amount + updated.get_unpaid_amount() == updated.total_cost
        ), (
            f"已付金额({updated.paid_amount}) + 未付金额({updated.get_unpaid_amount()}) 应该等于总成本({updated.total_cost})"
        )

        # 验证: 已付金额不超过总成本
        assert updated.paid_amount <= updated.total_cost, "已付金额不应该超过总成本"
    ...
    def test_payment_allocation_to_multiple_processing(
        self, db_manager, supplier_data, process_count
    ):
        """
