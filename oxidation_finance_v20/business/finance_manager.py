...
from ..models.business_models import (
    Income, Expense, ProcessingOrder, BankType, ExpenseType,
...
class FinanceManager:
    ...
    def allocate_payment_to_orders(
        ...
        for order_id, allocated_amount in allocations.items():
            ...
            if allocated_amount > unpaid_amount:
                return False, f"订单 {order.order_no} 的分配金额 {allocated_amount} 超过未付余额 {unpaid_amount}"
    ...
    def get_order_payment_status(self, order_id: str) -> Dict:
        ...
        unpaid_amount = order.total_amount - order.received_amount
        payment_ratio = (order.received_amount / order.total_amount * 100) if order.total_amount > 0 else 0
    ...
    def allocate_payment_to_expenses(
        ...
        if total_allocated > payment_amount:
            return False, f"分配金额总和 {total_allocated} 超过付款金额 {payment_amount}"
    ...
    def get_expense_summary_by_type(self, start_date: Optional[date] = None, 
                                    end_date: Optional[date] = None) -> Dict[str, Decimal]:
        """
        按类型汇总支出
    ...
    def get_professional_materials_expenses(self, start_date: Optional[date] = None,
    ...
    def update_account_balance(
        ...
        if is_income:
            ...
            if target_account.balance < amount:
                return False, f"账户余额不足：当前余额 {target_account.balance}，需要支出 {amount}"
            target_account.balance -= amount
    ...
    def match_transaction_to_income(
        ...
        if transaction.amount != income.amount:
            return False, f"金额不匹配：交易金额 {transaction.amount}，收入金额 {income.amount}"
        
        if transaction.bank_type != income.bank_type:
            return False, f"银行类型不匹配：交易银行 {transaction.bank_type.value}，收入银行 {income.bank_type.value}"
    ...
    def match_transaction_to_expense(
        ...
        if abs(transaction.amount) != expense.amount:
            return False, f"金额不匹配：交易金额 {abs(transaction.amount)}，支出金额 {expense.amount}"
        
        if transaction.bank_type != expense.bank_type:
            return False, f"银行类型不匹配：交易银行 {transaction.bank_type.value}，支出银行 {expense.bank_type.value}"
    ...
    def reconcile_bank_accounts(self) -> Dict:
        ...
        total_unmatched = g_bank_summary["unmatched_count"] + n_bank_summary["unmatched_count"]
    ...
    def record_accrual_income(
        ...
        if payment_date and payment_date != occurrence_date:
            ...
            if time_diff > 0:
                income.notes = f"{notes}; 预收款：提前{time_diff}天收款（付款日期：{payment_date.isoformat()}）" if notes else f"预收款：提前{time_diff}天收款（付款日期：{payment_date.isoformat()}）"
            else:
                income.notes = f"{notes}; 延迟收款：延后{abs(time_diff)}天收款（付款日期：{payment_date.isoformat()}）" if notes else f"延迟收款：延后{abs(time_diff)}天收款（付款日期：{payment_date.isoformat()}）"
    ...
    def record_accrual_expense(
        ...
        if payment_date and payment_date != occurrence_date:
            time_diff = (payment_date - occurrence_date).days
            if time_diff < 0:
                expense.notes = f"{notes}; 预付款：提前{abs(time_diff)}天付款（付款日期：{payment_date.isoformat()}）" if notes else f"预付款：提前{abs(time_diff)}天付款（付款日期：{payment_date.isoformat()}）"
            else:
                expense.notes = f"{notes}; 延迟付款：延后{time_diff}天付款（付款日期：{payment_date.isoformat()}）" if notes else f"延迟付款：延后{time_diff}天付款（付款日期：{payment_date.isoformat()}）"
    ...
    def match_income_to_expenses(
        ...
        for expense_id, allocated_amount in expense_allocations.items():
            ...
            expense.notes = f"{expense.notes}; {match_note}" if expense.notes else match_note
            self.db.save_expense(expense)
    ...
    def match_expense_to_incomes(
        self,
        expense_id: str,
        income_allocations: Dict[str, Decimal]
    ) -> Tuple[bool, str]:
        """
        灵活匹配支出到多个收入（支持多对多匹配）
        ...
        if total_allocated > expense.amount:
            return False, f"分配金额总和 {total_allocated} 超过支出金额 {expense.amount}"
        ...
        for income_id, allocated_amount in income_allocations.items():
            income = self.db.get_income(income_id)
            match_info += f" {income.customer_name}({income_id[:8]}): {allocated_amount}元;"
        
        expense.notes = f"{expense.notes}; {match_info}" if expense.notes else match_info
        self.db.save_expense(expense)
        ...
        for income_id, allocated_amount in income_allocations.items():
            ...
            income.notes = f"{income.notes}; {match_note}" if income.notes else match_note
            self.db.save_income(income)
    ...
    def get_accrual_period_summary(
        self,
        start_date: date,
        end_date: date
    ) -> Dict:
        ...
        period_incomes = [
            ...
            if start_date <= i.income_date <= end_date
        ]
        period_expenses = [
        ...
        # 按银行类型分类
        g_bank_income = sum(i.amount for i in period_incomes if i.bank_type == BankType.G_BANK)
        n_bank_income = sum(i.amount for i in period_incomes if i.bank_type == BankType.N_BANK)
        g_bank_expense = sum(e.amount for e in period_expenses if e.bank_type == BankType.G_BANK)
        n_bank_expense = sum(e.amount for e in period_expenses if e.bank_type == BankType.N_BANK)
    ...
    def log_operation(
        ...
        # 创建审计日志
        audit_log = AuditLog(
            operation_type=OperationType[operation_type] if isinstance(operation_type, str) and operation_type in OperationType.__members__ else operation_type,
            entity_type=EntityType[entity_type] if isinstance(entity_type, str) and entity_type in EntityType.__members__ else entity_type,
            entity_id=entity_id,
    ...
    def get_entity_audit_trail(self, entity_type: str, entity_id: str) -> List[Dict]:
        ...
        """
        return self.get_audit_logs(entity_type=entity_type, entity_id=entity_id, limit=1000)
    ...
    def get_operation_statistics(
        ...
        Returns:
            Dict: 操作统计信息
        """
        logs = self.db.list_audit_logs(start_time=start_date, end_time=end_date, limit=10000)
        ...
        for log in logs:
            op_type = log.operation_type.value if hasattr(log.operation_type, 'value') else str(log.operation_type)
    ...
    def adjust_accounting_period(
        ...
        for other_period in existing_periods:
            if other_period.id == period_id:
                continue
            if not (period.end_date < other_period.start_date or period.start_date > other_period.end_date):
                return False, f"调整后与期间 {other_period.period_name} 重叠"
        
        # 更新备注
        if notes:
            period.notes = f"{period.notes}; 调整：{notes}" if period.notes else f"调整：{notes}"
    ...
    def close_accounting_period(
        ...
        # 计算期间汇总数据
        period_summary = self.get_accrual_period_summary(period.start_date, period.end_date)
        ...
        if notes:
            period.notes = f"{period.notes}; 关闭：{notes}" if period.notes else f"关闭：{notes}"
    ...
    def reopen_accounting_period(
        ...
        if notes:
            period.notes = f"{period.notes}; 重新打开：{notes}" if period.notes else f"重新打开：{notes}"
    ...
    def get_current_accounting_period(self, reference_date: Optional[date] = None) -> Optional[Dict]:
        """
        获取当前会计期间（包含指定日期的期间）
